...
public class InitSessionRequestHandler extends SqlServiceHandler {
	
	private DataBaseSessionExternal dataBaseSessionHandler;
	
	public InitSessionRequestHandler(SqlServcieHandlerFactory sqlServcieHandlerFactory,ServiceRoute serviceRoute) {
	...
	public void handleRequestType(HttpServletRequest req, HttpServletResponse res)
		    ...
		    try
		    {
		    	this.getDataBaseSessionHandler().initSession(getSqlServcieHandlerResorce().getConnection());
		    }
		    catch(Exception e)
		    {
