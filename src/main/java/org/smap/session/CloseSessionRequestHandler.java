...
public class CloseSessionRequestHandler extends SqlServiceHandler {
	private DataBaseSessionExternal dataBaseSessionHandler;

	
	public CloseSessionRequestHandler(SqlServcieHandlerFactory sqlServcieHandlerFactory,ServiceRoute serviceRoute) {
	...
	public void handleRequestType(HttpServletRequest req, HttpServletResponse res)
		    ...
		    try
		    {
		    	this.getDataBaseSessionHandler().closeSession(getSqlServcieHandlerResorce().getConnection());
		    }
		    catch(Exception e)
		    {
